...
void Vigenere()
{
    ...
    for(i=0; i<10000; i++)
        tablica_ASCII[i]-=65;                    //Operacja, dzieki ktorej wiadomo o ile 'zmienic' (teskst do szyfrowania).
    licznik=0;
    ...
    if((in=fopen(plik_do_zaszyfrowania, "r"))!=NULL)  //Otwieranie pliku do zaszyfrowania.
    {
        if((out=fopen(plik_zaszyfrowany, "w+"))!=NULL) //Tworzenie pliku z zaszyfrowanym tesktem.
        {
            ...
            {                               //Znaki sa rozdzielane na male, badz wielkie litery.
                ...
                {
                    znak -= 65;
                    znak += tablica_ASCII[licznik];  //Operacja powodujaca przesuniecie zgodnie z teskstem i haslem (szyfrowanie).
